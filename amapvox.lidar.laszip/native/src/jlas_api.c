...
LASZIP_API laszip_I32 jlas_copy_point(laszip_point_struct* point_in, jlas_point_struct* point_out)
{
      ...
      if (point_in == 0) return 1;
      if (point_out == 0) return 2;
...
// static pointer for laszip_dll
static HINSTANCE laszip_HINSTANCE = NULL;

laszip_I32 laszip_load_dll_from_path(const char* full_path)
{
  ...
  // Load DLL file
#ifdef _WIN32
  laszip_HINSTANCE = LoadLibrary(TEXT(full_path));
#else
  laszip_HINSTANCE = LoadLibrary(full_path, RTLD_NOW);
#endif
  if (laszip_HINSTANCE == NULL) {
     return 1;
  }
  // Get function pointers
  return laszip_load_dll_functions(laszip_HINSTANCE);
}
...
laszip_I32 laszip_load_dll_functions(HINSTANCE laszip_HINSTANCE)
{
